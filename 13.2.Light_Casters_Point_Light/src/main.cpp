...
// -------------------------------------------------------------------------------------------------
    // 创建Window(窗口)
    GLFWwindow* window = glfwCreateWindow(1000, 720, "OpenGL_Study_Light_Casters_Point_Light", NULL, NULL);
    if(window == NULL) {
...
// -------------------------------------------------------------------------------------------------
    // Camera
    Camera camera(glm::vec3(0.0f, 10.0f, 12.0f), -40.0f, 180.0f, glm::vec3(0.0f, 1.0f, 0.0f));
// -------------------------------------------------------------------------------------------------
    // Cube_Model
    glm::mat4 model;
    glm::vec3 cube_position(0.0f, 0.0f, 0.0f);
    glm::vec3 cube_rotation(0.0f, 0.0f, 0.0f);
    glm::vec3 cube_scale(10.0f, 0.1f, 10.0);
    // View
    glm::mat4 view;
    // Projection
    glm::mat4 projection;
...
// -------------------------------------------------------------------------------------------------
        ...
        MyShader->setFloat("light.constant", light.constant);
        MyShader->setFloat("light.linear", light.linear);
        MyShader->setFloat("light.quadratic", light.quadratic);
        diffuseMap.Bind(0);
        specularMap.Bind(1);
        cube_VAO.Bind();
